...
def apply(keyboard):
    ...
    tap_time = 300
    LT_3SLSH = KC.LT(
        3, KC.SLSH, prefer_hold=True, tap_interrupted=False, tap_time=tap_time
    )
    LT_3Z = KC.LT(
        3, KC.Z, prefer_hold=True, tap_interrupted=False, tap_time=tap_time
    )
    LT_4SPC = KC.LT(
        4, KC.SPC, prefer_hold=True, tap_interrupted=False, tap_time=tap_time
    )
    LT_5TAB = KC.LT(
        5, KC.TAB, prefer_hold=True, tap_interrupted=False, tap_time=tap_time
    )
    LT_6ESC = KC.LT(
        6, KC.ESC, prefer_hold=True, tap_interrupted=False, tap_time=tap_time
    )
    LT_7BSPC = KC.LT(
        7, KC.BSPC, prefer_hold=True, tap_interrupted=False, tap_time=tap_time
    )
    LT_8ENT = KC.LT(
        8, KC.ENT, prefer_hold=True, tap_interrupted=False, tap_time=tap_time
    )
    LT_9DEL = KC.LT(
        9, KC.DEL, prefer_hold=True, tap_interrupted=False, tap_time=tap_time
    )
    MT_ALGUI = KC.MT(
        KC.A,
        KC.LGUI,
    ...
    mapping["Base"] = [
        ...
        KC.LCTL,     MT_ALGUI,    MT_SLALT,    MT_SLCTL,    MT_TLSFT,    KC.G,        KC.EQL,      KC.BSLS,     KC.M,        MT_NLSFT,    MT_ELCTL,    MT_ILALT,    MT_OLGUI,    KC.COLN,
        KC.LSFT,     LT_3Z,       MT_XRALT,    KC.C,        KC.D,        KC.V,        KC.LBRC,     KC.RBRC,     KC.K,        KC.H,        KC.COMM,     MT_DOTRALT,  LT_3SLSH,    KC.RSFT,
        ooooo,       ooooo,       KC.ESC,      LT_6ESC,     LT_4SPC,     LT_5TAB,     KC.ENTER,    KC.ESC,      LT_8ENT,     LT_7BSPC,    LT_9DEL,     KC.Q,        ooooo,       ooooo,
    ]
    mapping["Extra"] = [
        ...
        xxxxx,       MT_ALGUI,    MT_SLALT,    MT_SLCTL,    MT_TLSFT,    KC.G,        xxxxx,       xxxxx,       KC.H,        MT_NLSFT,    MT_ELCTL,    MT_ILALT,    MT_OLGUI,    xxxxx,
    ...
    sorted_layers = sorted(
        mapping.items(), key=lambda item: layer_names_list.index(item[0])
    )
        ...
        if extract:
            ...
            if len(keys) != key_width:
                for idx, k in enumerate(keys):
                    print(idx, k)
                raise ValueError(
                    f"Wrong number of keys in {current_block} on line {len(out[current_block]) // key_width}"
                )
