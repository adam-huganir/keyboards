...
parser.add_argument(
    "--comment", "-c", type=str, default="", help="Comment to add to the beginning drawn line"
)
parser.add_argument("--spacing", "-s", type=int, default=1, help="Add newline between keys")
            ...
            case (CC.EMPTY, CC.KEY, l, r):
                match l, r:
                    case (CC.JUNC, CC.KEY) | (CC.KEY, CC.JUNC) | (CC.KEY, CC.KEY):
                        line += TableChars.KEY
                    case (
                        (CC.EMPTY, CC.JUNC) | (CC.JUNC, CC.EMPTY) | (CC.EMPTY, CC.EMPTY)
                    ):
                        line += TableChars.KEY
                    case (CC.EMPTY, CC.KEY) | (CC.KEY, CC.EMPTY):
                        line += TableChars.BM
